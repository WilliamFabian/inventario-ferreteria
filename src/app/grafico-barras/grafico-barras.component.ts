...
export class GraficoBarrasComponent {
  ...
  async generarGraficoId() {
    const productos = await firstValueFrom(this.productosService.obtenerRegistros('productos'));
    const ventas = await firstValueFrom(this.productosService.obtenerRegistros('ventas'));
